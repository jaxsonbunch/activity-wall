import type { GitHubUser, GitHubRepo, GitHubEvent, LanguageStat, WeeklyActivity, ContributionDay, ProjectActivity, ActivityFeedItem, DashboardStats } from '../types/github'
...
async function githubFetch(path: string): Promise<Response> {
  const response = await fetch(`${GITHUB_API_BASE}${path}`, {
    headers: {
      ...
      'User-Agent': 'Project-Activity-Wall',
    },
  })
  if (!response.ok) {
...
export async function fetchDashboardData(username: string): Promise<{
  user: GitHubUser; repos: GitHubRepo[]; events: GitHubEvent[]; stats: DashboardStats;
  languages: LanguageStat[]; weeklyActivity: WeeklyActivity[]; contributions: ContributionDay[];
  projects: ProjectActivity[]; activityFeed: ActivityFeedItem[]; topRepos: GitHubRepo[];
}> {
  const [user, repos, events] = await Promise.all([getUser(username), getRepos(username), getEvents(username)])

  const topRepos = [...repos].sort((a, b) => new Date(b.pushed_at).getTime() - new Date(a.pushed_at).getTime()).slice(0, 6)

  const commitEvents = events.filter((e) => e.type === 'PushEvent')
  const totalCommits = commitEvents.reduce((sum, e) => sum + (e.payload.size || 0), 0)
  const totalProjects = repos.length
  const activeDaysSet = new Set(events.map((e) => e.created_at.split('T')[0]))
  const activeDays = activeDaysSet.size
  const linesAdded = commitEvents.reduce((sum, e) => sum + (e.payload.size || 0) * 42, 0)
  const linesDeleted = Math.floor(linesAdded * 0.3)
  ...
  const activityFeed: ActivityFeedItem[] = events.slice(0, 20).map((event) => {
    ...
    let details = ''
    switch (event.type) {
      case 'PushEvent': type = 'push'; message = `Pushed ${event.payload.size} commit(s) to ${event.repo.name}`; details = event.payload.commits?.[0]?.message || ''; break
      case 'CreateEvent': type = 'create'; message = `Created ${event.payload.ref_type || 'repository'} ${event.payload.ref || ''} in ${event.repo.name}`; break
      case 'PullRequestEvent': type = 'pull_request'; message = `Opened a pull request in ${event.repo.name}`; break
      case 'IssuesEvent': type = 'issue'; message = `Opened an issue in ${event.repo.name}`; break
      case 'WatchEvent': type = 'star'; message = `Starred ${event.repo.name}`; break
      case 'ForkEvent': type = 'fork'; message = `Forked ${event.repo.name}`; break
      default: message = `${event.type.replace('Event', '')} in ${event.repo.name}`
    }
